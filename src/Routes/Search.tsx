...
function Search() {
  ...
  } = useQuery<IGetTvShowsResult>(['tv', keyword], () =>
  ...
  const clickedItem =
    ...
    (movies?.results.find(
      (movie) => String(movie.id) === bigSearchMatch.params.id,
    ) ||
      tvShows?.results.find((tv) => String(tv.id) === bigSearchMatch.params.id))
  return (
    <Wrapper>
      {moviesError || tvShowsError ? (
        <InfoMessage>Internal Server Error.</InfoMessage>
      ) : moviesLoading || tvShowsLoading ? (
        <Loader>Loading...</Loader>
      ) : movies?.results.length === 0 && tvShows?.results.length === 0 ? (
        <InfoMessage>No results were found for your search.</InfoMessage>
      ) : (
        <>
          <InfoMessage>
                ...
                >
                  {clickedItem && (
                    <>
                      ...
                      <BigInfo>
                        {clickedItem.release_date ? (
                          <span>Release date : {clickedItem.release_date}</span>
                        ) : (
                          <span>
                            First air date : {clickedItem.first_air_date}
                          </span>
                        )}
                        <span>Rating : {clickedItem.vote_average}</span>
                      </BigInfo>
                    </>
                  )}
                </BigItem>
